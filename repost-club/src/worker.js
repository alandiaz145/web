...
async function expireCampaigns(env) {
  const { results = [] } = await env.DB.prepare(`
    SELECT p.id,p.user_id,COALESCE(SUM((a.target_count-a.completed_count)*a.reward_points),0) refund
  ...
  for (const post of results) {
    ...
    if (refund > 0) {
      await env.DB.batch([
        ...
        env.DB.prepare(`INSERT INTO point_transactions(user_id,amount,type,post_id,note) VALUES(?,?,'campaign_refund',?,'Créditos no utilizados al vencer la campaña')`).bind(post.user_id, refund, post.id)
      ]);
    }
  }
}
...
async function api(request, env, url) {
  ...
  if (request.method === 'POST' && url.pathname === '/api/register') {
    ...
    try {
      ...
      `).bind(username, email, await hashPassword(password), instagram).first();
      const token = randomToken();
      await env.DB.batch([
        env.DB.prepare(`INSERT INTO point_transactions(user_id,amount,type,note) VALUES(?,50,'signup_bonus','Saldo inicial')`).bind(created.id),
        env.DB.prepare(`INSERT INTO sessions(token,user_id,expires_at) VALUES(?,?,datetime('now','+30 days'))`).bind(token, created.id)
      ]);
  ...
  if (request.method === 'GET' && url.pathname === '/api/feed') {
    const { results = [] } = await env.DB.prepare(`
      ...
      FROM posts p JOIN users u ON u.id=p.user_id JOIN post_actions a ON a.post_id=p.id
      WHERE p.status='active' AND p.expires_at>datetime('now')
  ...
  if (request.method === 'POST' && url.pathname === '/api/posts') {
    ...
    for (const a of actions) {
      ...
      clean.push({ type,target,reward:prices[type],verification:type === 'comment' ? 'automatic' : 'trust' });
    }
    ...
    let post;
    try {
      post = await env.DB.prepare(`INSERT INTO posts(user_id,platform,url,title,expires_at) VALUES(?,?,?,?,datetime('now','+24 hours')) RETURNING id,url,title,platform,created_at,expires_at`).bind(user.id,platform,urlValue,title).first();
  ...
  if (request.method === 'POST' && completeMatch) {
    ...
    let participation;
    try {
      participation = await env.DB.prepare(`INSERT INTO participations(post_action_id,user_id,status) VALUES(?,?,'claimed') RETURNING id`).bind(actionId,user.id).first();
    } catch (e) {
    ...
    `).bind(actionId).first();
    if (!slot) {
      await env.DB.prepare('DELETE FROM participations WHERE id=?').bind(participation.id).run();
    ...
    try {
      ...
      await env.DB.batch([
        ...
        env.DB.prepare('DELETE FROM participations WHERE id=?').bind(participation.id)
      ]);
  ...
  if (request.method === 'GET' && url.pathname === '/api/my-posts') {
    const { results = [] } = await env.DB.prepare(`
      SELECT p.*,COALESCE(SUM(a.target_count*a.reward_points),0) budget,COALESCE(SUM(a.completed_count*a.reward_points),0) spent
      FROM posts p LEFT JOIN post_actions a ON a.post_id=p.id
